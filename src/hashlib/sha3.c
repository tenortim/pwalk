...
{
  SHA3 *sha3 = malloc(sizeof(SHA3));
  if (sha3 == NULL) {
    return NULL;
  }
  sha3->m_blockSize = (200 - 2 * (bits / 8));
  // validate bits
...
static const unsigned int Rounds = 24;
...
static void processBlock(SHA3 *self, const void* data)
{
...
#define LITTLEENDIAN(x) (x)
  ...
  for (unsigned int round = 0; round < Rounds; round++)
  {
    ...
    // Chi
    for (unsigned int j = 0; j < SHA3_StateSize; j += 5)
    {
      // temporaries
...
static void processBuffer(SHA3 *self)
{
  // add padding
  size_t offset = self->m_bufferSize;
  // add a "1" byte
  self->m_buffer[offset++] = 0x06;
  // fill with zeros
  while (offset < self->m_blockSize)
    self->m_buffer[offset++] = 0;

  // and add a single set bit
  self->m_buffer[offset - 1] |= 0x80;
...
{
  // save hash state
  uint64_t oldHash[SHA3_StateSize];
  for (unsigned int i = 0; i < SHA3_StateSize; i++)
...
{
  SHA3_reset(self);
  SHA3_add(self, data, numBytes);
  return SHA3_getHash_str(self);
}
...
{
  SHA3_reset(self);
  SHA3_add(self, text, strlen(text));
  return SHA3_getHash_str(self);
}

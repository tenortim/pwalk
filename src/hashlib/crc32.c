...
CRC32 *CRC32_init()
{
  CRC32 *crc32 = malloc(sizeof(CRC32));
  if (crc32 == NULL) {
    return NULL;
  }
  CRC32_reset(crc32);
...
char *CRC32_getHash_str(CRC32 *self)
{
  ...
  hashBuffer[0] = dec2hex[ self->m_hash >> 28      ];
  hashBuffer[1] = dec2hex[(self->m_hash >> 24) & 15];
  hashBuffer[2] = dec2hex[(self->m_hash >> 20) & 15];
  hashBuffer[3] = dec2hex[(self->m_hash >> 16) & 15];
  hashBuffer[4] = dec2hex[(self->m_hash >> 12) & 15];
  hashBuffer[5] = dec2hex[(self->m_hash >>  8) & 15];
  hashBuffer[6] = dec2hex[(self->m_hash >>  4) & 15];
  hashBuffer[7] = dec2hex[ self->m_hash        & 15];
...
void CRC32_getHash(CRC32 *self, unsigned char buffer[CRC32_HashBytes])
{
  buffer[0] = (self->m_hash >> 24) & 0xFF;
  buffer[1] = (self->m_hash >> 16) & 0xFF;
  buffer[2] = (self->m_hash >>  8) & 0xFF;
  buffer[3] =  self->m_hash        & 0xFF;
}
...
{
  CRC32_reset(self);
  CRC32_add(self, data, numBytes);
  return CRC32_getHash_str(self);
}
...
{
  CRC32_reset(self);
  CRC32_add(self, text, strlen(text));
  return CRC32_getHash_str(self);
}
